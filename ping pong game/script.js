...
function resetBoard(paddleName){
    ...
    paddletop.style.left = (window.innerWidth - paddletop.offsetHeight) / 2 + 'px';
    paddlebottom.style.left = (window.innerWidth - paddlebottom.offsetHeight) / 2 + 'px';
...
//update maximum score which are store 
    ...
    alert(paddle + " wins with a score of " + (score * 100) + ". Max score is: " +  (maxScore * 100));
}
...
window.addEventListener('keypress', function(){
    ...
    if (event.code === "KeyA" && ((paddleRect.x + paddleRect.width) < window.innerWidth)) {
    ...
    if(event.code === "Enter"){
        ...
        if(!gameStart){
            ...
            movement = this.setInterval(function(){
                ...
                paddletopX = paddletop.getBoundingClientRect().x;
                paddlebottomX = paddlebottom.getBoundingClientRect().x;
                ...
                if ((ballPos < paddlebottomX) || (ballPos > (paddlebottomX + paddlebottomWidth))){
